...
public interface UsageMetricsRepository extends ReactiveMongoRepository<UsageMetrics, String> {
    ...
    Flux<UsageMetrics> findByServiceId(String serviceId);
    ...
    Flux<UsageMetrics> findByStartTimeAfter(LocalDateTime startTime);
    
    @Query("{ 'endTime': { $lte: ?0 } }")
    Flux<UsageMetrics> findByEndTimeBefore(LocalDateTime endTime);
    
    @Query("{ 'userId': ?0, 'serviceType': ?1 }")
    Flux<UsageMetrics> findByUserIdAndServiceType(String userId, UsageMetrics.ServiceType serviceType);
    
    @Query("{ 'createdAt': { $gte: ?0, $lte: ?1 } }")
    Flux<UsageMetrics> findByCreatedAtBetween(LocalDateTime start, LocalDateTime end);
}