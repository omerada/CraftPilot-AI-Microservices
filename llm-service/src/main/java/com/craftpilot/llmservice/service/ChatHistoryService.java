...
public class ChatHistoryService {
    ...
    public Mono<ChatHistory> getChatHistoryById(String id) {
        ...
        return chatHistoryRepository.findById(id)
                .doOnError(
                        error -> log.error("ID ile sohbet geçmişi getirirken hata, ID {}: {}", id, error.getMessage()))
                .onErrorResume(e -> Mono.empty());
    }

    @LogActivity(actionType = ActivityEventTypes.CHAT_HISTORY_CREATE, userIdParam = "#chatHistory.userId", metadata = "{\"id\": #result.id, \"title\": #result.title}")
    ...
    @LogActivity(actionType = ActivityEventTypes.CHAT_HISTORY_UPDATE, userIdParam = "#chatHistory.userId", metadata = "{\"id\": #result.id, \"title\": #result.title, \"messageCount\": #result.conversations.size()}")
    public Mono<ChatHistory> updateChatHistory(ChatHistory chatHistory) {
        ...
        return chatHistoryRepository.save(chatHistory)
                .doOnError(error -> log.error("Sohbet geçmişi güncellenirken hata, ID {}: {}", chatHistory.getId(),
                        error.getMessage()))
                .onErrorMap(e -> new RuntimeException("Sohbet geçmişi güncellenemedi: " + e.getMessage(), e));
    }

    @LogActivity(actionType = ActivityEventTypes.CHAT_HISTORY_DELETE, userIdParam = "#userId", metadata = "{\"id\": #historyId}")
    ...
    @LogActivity(actionType = ActivityEventTypes.CONVERSATION_CREATE, userIdParam = "#result.userId", metadata = "{historyId: #result.id, conversationId: #result.conversations[#result.conversations.size()-1].id}")
    ...
    @LogActivity(actionType = ActivityEventTypes.TITLE_UPDATE, userIdParam = "#result.userId", metadata = "{historyId: #result.id, newTitle: #result.title}")
    public Mono<ChatHistory> updateChatHistoryTitle(String historyId, String newTitle) {
        ...
        return chatHistoryRepository.updateTitle(historyId, newTitle)
                .doOnError(error -> log.error("Sohbet başlığı güncellenirken hata, ID {}: {}", historyId,
                        error.getMessage()))
                .onErrorMap(e -> new RuntimeException("Sohbet başlığı güncellenemedi: " + e.getMessage(), e));
    }

    @LogActivity(actionType = ActivityEventTypes.CHAT_HISTORY_ARCHIVE, userIdParam = "#userId", metadata = "{\"id\": #historyId}")
    ...
    @LogActivity(actionType = ActivityEventTypes.CHAT_HISTORY_UNARCHIVE, userIdParam = "#userId", metadata = "{\"id\": #historyId}")
    ...
    /**
     * Belirli kullanıcının arşivlediği sohbet geçmişlerini sayfalanmış olarak
     * getirir
     */
    ...
    public Mono<PaginatedChatHistoryResponse> getChatHistoriesByUserIdCategorized(
        ...
        return chatHistoryRepository.findAllByUserId(userId, 1, Integer.MAX_VALUE) // Get all histories first
                ...
                .flatMap(allHistories -> {
                    ...
                    // Kategori boyutlarını ayrı bir map'te topla - bu sayede doğru toplam değerleri
                    // hesaplanacak
                    ...
                    // Sayfalama için skip ve limit değerini hesapla
                    int skipCount = (page - 1) * pageSize;
                    int remainingItems = Math.min(pageSize, totalFilteredRecords - skipCount);
                    ...
                    if (remainingItems > 0) {
                        for (String category : finalCategoryFilters) {
                            List<ChatHistory> histories = categorizedHistories.getOrDefault(category,
    ...
    private long getTimestampValue(LocalDateTime timestamp) {
        return timestamp != null ? timestamp.toEpochSecond(ZoneOffset.UTC) : 0L;
    }
    ...
    private Map<String, List<ChatHistory>> categorizeHistories(List<ChatHistory> histories) {
        ...
        for (ChatHistory history : histories) {
            ...
            LocalDateTime timestamp = history.getUpdatedAt() != null ? history.getUpdatedAt() : history.getCreatedAt();
            ...
            // LocalDateTime'i LocalDate'e dönüştür
            LocalDate historyDate = timestamp.toLocalDate();
    ...
    private LocalDate getLocalDateFromTimestamp(LocalDateTime timestamp) {
    ...
    private Map<String, Object> convertToChatGPTFormat(ChatHistory history) {
        ...
        if (history.getCreatedAt() != null) {
            // LocalDateTime'i ISO formatına dönüştür
            item.put("create_time", history.getCreatedAt().toString());
        }

        if (history.getUpdatedAt() != null) {
            item.put("update_time", history.getUpdatedAt().toString());
        }
