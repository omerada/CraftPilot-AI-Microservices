...
public class RedisCacheService {
    ...
    /**
     * Kullanıcı tercihlerini cache'den alır, yoksa veri kaynağından yükler ve
     * cache'e kaydeder
     * 
    ...
    /**
     * Kullanıcı tercihlerini cache'den alır, yoksa veri kaynağından yükler ve
     * belirtilen süre için cache'e kaydeder
     * 
     * @param userId Kullanıcı ID
     * @param loader Veri kaynağından yükleme fonksiyonu
    ...
    public Mono<UserPreference> cacheUserPreference(String userId, UserPreference userPreference, Duration ttl) {
        ...
        return redisTemplate.opsForValue().set(key, userPreference, CACHE_TTL)
                ...
                .doOnSuccess(p -> log.debug("Kullanıcı tercihleri başarıyla önbelleğe kaydedildi: userId={}",
                        userPreference.getUserId()))
                .doOnError(error -> log.error("Kullanıcı tercihleri önbelleğe kaydedilirken hata: {}",
                        error.getMessage()));
    }
    ...
    public Mono<Void> invalidateUserPreference(String userId) {
        ...
        return redisTemplate.delete(key)
                .doOnSuccess(deleted -> log.debug("Kullanıcı tercihleri önbellekten silindi: userId={}, deleted={}",
                        userId, deleted))
                .doOnError(
                        error -> log.error("Kullanıcı tercihleri önbellekten silinirken hata: {}", error.getMessage()))
                .then();
    }
